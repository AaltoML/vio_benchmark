...
int main(int argc, char **argv)
{
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            cv::Mat tm = SLAM.TrackStereo(imLeftRect,imRightRect,tframe,vImuMeas);
            ...
            if (tm.rows == 4 && tm.cols == 4) {
                ...
                // g2o::SE3Quat pose = ORB_SLAM3::Converter::toSE3Quat(tm);
                // outputJson["position"]["x"] = tm.at<float>(0, 3);
                // outputJson["position"]["y"] = tm.at<float>(1, 3);
                // outputJson["position"]["z"] = tm.at<float>(2, 3);
                // Eigen::Vector3d translation = pose.translation();
                // Eigen::Quaterniond rotation = pose.rotation();
                // outputJson["position"]["x"] = translation.x();
                // outputJson["position"]["y"] = translation.y();
                // outputJson["position"]["z"] = translation.z();
